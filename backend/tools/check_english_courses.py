...
with app.app_context():
    results = db.session.execute(
        course_course_type.select().where(course_course_type.c.course_type == "英语必修")
    ).fetchall()
    print(f"英语必修课程数量: {len(results)}")
    for r in results:
    ...
    print("\n--- 所有课程类型 ---")
    all_types = db.session.execute(course_course_type.select()).fetchall()
    types = set(r.course_type for r in all_types)
    print(sorted(types))
